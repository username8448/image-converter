import { canvasContext2d, errorMessage, requireElement, requireQuery } from '../../shared/js/dom.ts';

type AsciiSource = HTMLImageElement | HTMLCanvasElement;
type GifMode = 'none' | 'frames' | 'image';
type CharBrightness = {
  readonly ch: string;
  readonly brightness: number;
};
type AsciiControlsSnapshot = {
...
type AsciiSavedState = {
  readonly isGif: false;
  readonly mode: string;
  readonly controls: AsciiControlsSnapshot;
  readonly lastAscii: string;
  readonly lastImageData: ImageDataSnapshot | null;
  readonly previewDataUrl: string;
};
type AsciiQueueItem = FileEntry & {
  readonly key: string;
  savedState?: AsciiSavedState | null;
};
type HandleFileOptions = {
  readonly restoreState?: AsciiSavedState | null;
};
...
let lastAscii = '';
let lastImageData: ImageDataSnapshot | null = null;
...
let gifMode: GifMode = 'none';
let currentFile: File | null = null;
let currentFileName = '';
let batchQueue: AsciiQueueItem[] = [];
let batchActiveKey = '';
let batchBusy = false;
...
const charBrightnessCache = new Map<string, number>();
let contrastOptimizedCharset: string | null = null;

const brightnessCanvas = document.createElement('canvas');
brightnessCanvas.width = 32;
brightnessCanvas.height = 32;
const brightnessCtx = canvasContext2d(brightnessCanvas, { willReadFrequently: true });

function measureCharBrightness(char: string): number {
...
function initPasteSupport(): void {
...
async function handleFile(file: File | null, options: HandleFileOptions = {}): Promise<void> {
  if (!file) {
...
function removeImage(e?: Event | null, options: RemoveImageOptions = {}): void {
...
function getBatchQueueItems(): AsciiQueueItem[] {
  return batchQueue.slice();
}

function findQueueItemByKey(key: string): AsciiQueueItem | null {
  return batchQueue.find((item) => item.key === key) || null;
}

function captureAsciiControls(): AsciiControlsSnapshot {
  return {
...
function applyAsciiControls(snapshot: AsciiControlsSnapshot | null | undefined): void {
  if (!snapshot) return;
...
function captureCurrentAsciiState(): AsciiSavedState | null {
...
function drawPreviewCanvasFromDataUrl(dataUrl: string): Promise<boolean> {
...
async function restoreAsciiStateFromCache(state: AsciiSavedState | null | undefined): Promise<boolean> {
...
function loadQueueItem(item: AsciiQueueItem): void {
  batchActiveKey = item.key;
  if (item.savedState && !item.savedState.isGif) {
    void handleFile(item.file, { restoreState: item.savedState });
  } else {
...
function addFilesToQueue(entries: readonly FileEntry[], sourceLabel?: string): void {
  ...
  let added = 0;
  let skipped = 0;
  let firstAdded: AsciiQueueItem | null = null;
...
const ctx = canvasContext2d(hiddenCanvas, { willReadFrequently: true });
const ASCII_ASPECT_RATIO = 0.68;
const ASCII_FONT_SIZE = 10;
const ASCII_FONT = `${ASCII_FONT_SIZE}px Consolas, "Courier New", monospace`;
const asciiMeasureCanvas = document.createElement('canvas');
const asciiMeasureCtx = canvasContext2d(asciiMeasureCanvas);
...
function imageToAscii(img: AsciiSource, width: number, charSet: string): string {
...
function applyFloydSteinbergDithering(
  ...
  const addError = (index: number, value: number): void => {
...
function fitAsciiToScreen(): void {
...
function renderColorPreview(): void {
...
function renderAsciiToCanvas(asciiText: string, color: string): HTMLCanvasElement {
...
function buildAsciiCanvasFromText(color: string): HTMLCanvasElement {
  return renderAsciiToCanvas(lastAscii, color);
}

function renderColoredAsciiFrame(asciiText: string, imageData: ImageDataSnapshot): HTMLCanvasElement {
  const lines = asciiText.split('\n');
  const tempCanvas = document.createElement('canvas');
  const tempCtx = canvasContext2d(tempCanvas);
...
function setGifSaveUiState(label: string, disabled: boolean): void {
  elements.btnSavePng.textContent = label;
  elements.btnSavePng.disabled = disabled;
}

async function saveAsciiGif(): Promise<void> {
  if (!isGif) {
  ...
  try {
    ...
    for (const frame of gifFrames) {
      ...
      const canvas = color === 'original'
        ? renderColoredAsciiFrame(asciiText, frameImageData)
...
function getCharSetValue(): string {
...
async function copyPngToClipboard(canvas: HTMLCanvasElement): Promise<void> {
...
function renderColorAsciiCanvas(asciiText: string, imageData: ImageDataSnapshot | null): HTMLCanvasElement {
...
function buildAsciiZipEntryName(queueItem: AsciiQueueItem): string {
...
async function createAsciiPngBlobForFile(file: File): Promise<Blob> {
  const width = parseInt(elements.widthInput.value, 10);
...
function openPreviewFromCurrentOutput(): void {
  const color = elements.colorSelect.value;
...
function performConversion(img: AsciiSource | null = currentImage): void {
  if (!img) return;
...
function handleCharsetChange(): void {
