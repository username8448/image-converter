// Generated by deno task build from src/shared/js/app-core.ts.
// Do not edit this file directly; edit the TypeScript source instead.
(() => {
  // src/shared/js/app-core.ts
  (function initAppCoreRuntime() {
    ...
    function isBrowserFileEntry(entry) {
      return entry.isFile && typeof entry.file === "function";
    }
    function isBrowserDirectoryEntry(entry) {
      return entry.isDirectory && typeof entry.createReader === "function";
    }
    function normalizePath(path) {
      return String(path || "").replaceAll("\\\\", "/").replace(/^\.?\//, "").replace(/\/+/g, "/").trim();
    }
    ...
    function sanitizePath(path) {
      ...
      const segments = normalized.split("/").filter(Boolean).map((segment) => sanitizeFileName(segment)).filter((segment) => segment !== "." && segment !== "..");
    ...
    async function readEntryRecursive(entry, prefixPath) {
      ...
      if (isBrowserFileEntry(entry)) {
        ...
        return [
          {
            file,
            relativePath: `${prefixPath}${file.name}`
          }
        ];
      }
    ...
    function crc32(bytes) {
      ...
      for (let i = 0; i < bytes.length; i += 1) {
        ...
        crc = (CRC32_TABLE[(crc ^ byte) & 255] ?? 0) ^ crc >>> 8;
      }
    ...
    function buildZipBlob(entries) {
      ...
      for (const entry of safeEntries) {
        ...
        writeU32LE(localHeader, 0, 67324752);
        writeU16LE(localHeader, 4, 20);
        writeU16LE(localHeader, 6, 2048);
        ...
        writeU32LE(centralHeader, 0, 33639248);
        writeU16LE(centralHeader, 4, 20);
        writeU16LE(centralHeader, 6, 20);
        writeU16LE(centralHeader, 8, 2048);
      ...
      writeU32LE(endRecord, 0, 101010256);
    ...
    function bindIngestControls(options) {
      const { fileInput, folderInput, dropZone, pickFilesBtn, pickFolderBtn, clearQueueBtn, ignoreSelector, onEntries, onFile, onClear, isBusy } = options || {};
    ...
    function bindQueueList(options) {
      const { queueList, onSelect, onRemove, isBusy, selectAttr = "data-select-key", removeAttr = "data-remove-key", decodeSelect = decodeURIComponent, decodeRemove = decodeURIComponent } = options || {};
      if (!queueList) return;
      const busyCheck = typeof isBusy === "function" ? isBusy : () => false;
    ...
    function renderQueue(options) {
      const { queueList, items, emptyText = "\u0414\u043E\u0431\u0430\u0432\u044C\u0442\u0435 \u0444\u0430\u0439\u043B\u044B \u0434\u043B\u044F \u043E\u0431\u0440\u0430\u0431\u043E\u0442\u043A\u0438", isBusy = false, isActive, getItemId, getName, getPath, getBadgeClass, getBadgeText, showRemove = true, removeDisabled, selectAttr = "data-select-key", removeAttr = "data-remove-key", encodeId = encodeURIComponent } = options || {};
      if (!queueList) return;
      ...
      const rows = list.map((item, index) => {
        ...
        const disableRemove = typeof removeDisabled === "function" ? Boolean(removeDisabled(item, index)) : Boolean(removeDisabled || isBusy);
        ...
        <div class="queue-item${activeClass}" ${selectAttr}="${id}">
          ...
          ${showRemove ? `<button type="button" class="queue-remove" ${removeAttr}="${id}" ${removeDisabledAttr}>\xD7</button>` : ""}
        </div>
      `;
      });
    ...
    function updateIngestButtons(options) {
      const { pickFilesBtn, pickFolderBtn, clearQueueBtn, fileInput, folderInput, zipBtn, busy, queueLength, disableZipWhenQueueEmpty = true } = options || {};
    ...
    const fileBufferCache = /* @__PURE__ */ new WeakMap();
    ...
    async function decodeGifWithImageDecoder(buffer, token, options) {
      ...
      const frameInterval = options && options.frameInterval ? options.frameInterval : 1e3 / 24;
      ...
      for (let i = 0; i < track.frameCount; i++) {
        ...
        const durationMs = frame.duration ? frame.duration / 1e3 : frameInterval;
        if (frame && typeof frame.close === "function") frame.close();
        frames.push({
          canvas: frameCanvas,
          delay: Math.max(durationMs || frameInterval, frameInterval)
        });
      }
      if (typeof decoder.close === "function") decoder.close();
